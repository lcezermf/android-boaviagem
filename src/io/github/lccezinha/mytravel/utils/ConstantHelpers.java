package io.github.lccezinha.mytravel.utils;

public class ConstantHelpers {
	public static final String TRAVEL_MODE = "travel_mode";
	public static final String TRAVEL_ID = "travel_id";
	public static final String TRAVEL_DESTINY = "travel_destiny";
	public static final String TRAVEL_SELECTED = "travel_selected";
	public static final String SELECT_TRAVEL_MODE = "select_travel_mode";
	public static final int LEISURE_TRAVEL = 1;
	public static final int BUSINESS_TRAVEL = 2;
}
